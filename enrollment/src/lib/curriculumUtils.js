...
export function jsonToXlsxBlob(data) {
  ...
  if (items.length === 0) {
    throw new Error("No curriculum data to export");
  }

  const workbook = XLSX.utils.book_new();

  // Handle array of multiple year curriculum objects or single year object
  const yearOrder = ["1st", "2nd", "3rd", "4th"];
  const yearMap = new Map();

  items.forEach((item) => {
    const rawYear = item?.year || item?.data?.year;
    const normYear = toYearKey(rawYear);
    const docData = item?.data || item;
    if (docData && (Array.isArray(docData.semesters) || Array.isArray(docData.subjects))) {
      yearMap.set(normYear || "1st", docData);
    }
  });

  if (yearMap.size === 0 && items[0]) {
    yearMap.set("1st", items[0].data || items[0]);
  }

  // Sort sheets by 1st_sem, 2nd_sem, 3rd_sem, 4th_sem
  const keysToExport = yearOrder.filter((y) => yearMap.has(y));
  if (keysToExport.length === 0) {
    keysToExport.push(...Array.from(yearMap.keys()));
  }

  keysToExport.forEach((yKey) => {
    const doc = yearMap.get(yKey);
    const rows = buildSplitTemplateRows({ ...doc, year: yKey });
    const sheetName = `${yKey}_sem`;
...
function toYearKey(raw) {
  ...
  if (str.includes("1st") || str.includes("1") || str.includes("first")) return "1st";
  if (str.includes("2nd") || str.includes("2") || str.includes("second")) return "2nd";
  if (str.includes("3rd") || str.includes("3") || str.includes("third")) return "3rd";
  if (str.includes("4th") || str.includes("4") || str.includes("fourth")) return "4th";
  return null;
}

/**
 * Convert template XLSX file content (ArrayBuffer) to nested JSON array of year data.
 */
export function xlsxToJson(fileBuffer) {
  ...
  const detectYear = (sheetName, firstCell, index) => {
    const sName = String(sheetName || "").trim().toLowerCase();
    const fCell = String(firstCell || "").trim().toLowerCase();

    if (sName.includes("1st") || sName.includes("1") || fCell.includes("first")) return "1st";
    if (sName.includes("2nd") || sName.includes("2") || fCell.includes("second")) return "2nd";
    if (sName.includes("3rd") || sName.includes("3") || fCell.includes("third")) return "3rd";
    if (sName.includes("4th") || sName.includes("4") || fCell.includes("fourth")) return "4th";

    const defaultYears = ["1st", "2nd", "3rd", "4th"];
    return defaultYears[index] || "1st";
  };
  ...
  for (let i = 0; i < workbook.SheetNames.length; i++) {
    ...
    const detectedYear = detectYear(sheetName, rows[0]?.[0], i);
    const splitTemplateParsed = parseSplitTemplateRows(rows);
    const templateParsed = splitTemplateParsed || parseTemplateCsv(rows);
  ...
  if (!curriculaList.length) {
    throw new Error("Unsupported XLSX format or no valid curriculum data found");
  }
...
function buildSplitTemplateRows(doc) {
  ...
  const rows = [
    ...
    ["", "Code", "Title", "Lec", "Lab", "Units", "Hours", "", "Code", "Title", "Lec", "Lab", "Units", "Hours"],
  ];

  let lec1 = 0, lab1 = 0, units1 = 0, hours1 = 0;
  let lec2 = 0, lab2 = 0, units2 = 0, hours2 = 0;
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    if (right) {
      ...
      hours2 += Number.isFinite(rightHours) ? rightHours : 0;
    }
  ...
  rows.push(["", "", "", lec1, lab1, units1, hours1, "", "", "", "", lec2, lab2, units2, hours2]);
  return rows;
}
...
function parseSplitTemplateRows(rows) {
  ...
  for (let i = columnsRowIndex + 1; i < rows.length; i++) {
    ...
    const extractPrereqs = (title, existing) => {
      if (Array.isArray(existing) && existing.length > 0) return existing;
      const match = String(title || "").match(/\(pre:\s*([^)]+)\)/i);
      if (match && match[1]) {
    ...
    if ((leftCode || leftTitle) && leftCode.toLowerCase() !== "total") {
      const parsedPrereqs = parsePrerequisites(row[leftColumns.prerequisites]);
      firstSubjects.push({
        ...
        prerequisites: extractPrereqs(leftTitle, parsedPrereqs),
      });
    }

    if ((rightCode || rightTitle) && rightCode.toLowerCase() !== "total") {
      const parsedPrereqs = parsePrerequisites(row[rightColumns.prerequisites]);
      secondSubjects.push({
        ...
        prerequisites: extractPrereqs(rightTitle, parsedPrereqs),
      });
    }
  }
