...
import { xlsxToJson, jsonToCsv, jsonToXlsxBlob, downloadFile, downloadBlob } from "../lib/curriculumUtils.js";
...
export default function CurriculumTable({ onRegularImportSuccess }) {
  ...
  const parseCurriculumImportFile = async (file) => {
    ...
    if (!isXlsx) {
      throw new Error("Please select a valid XLSX file");
    }

    const buffer = await file.arrayBuffer();
    const parsedCurricula = xlsxToJson(buffer);

    if (!Array.isArray(parsedCurricula) || parsedCurricula.length === 0) {
      throw new Error("Invalid curriculum template format");
    }

    return parsedCurricula;
  };
  ...
  const handleImport = async (e) => {
    ...
    try {
      setIsUploading(true);
      const parsedCurricula = await parseCurriculumImportFile(file);

      const payload = {
        curricula: parsedCurricula,
      };

      await api.post("/curriculum", payload);
      toast.success("Curriculum for all year levels updated successfully!");
  ...
  const handleExport = async () => {
    try {
      ...
      const allCurricula = Array.isArray(res.data) ? res.data : [];
      if (!allCurricula.length) {
        toast.error("No curriculum data found to export");
        return;
      }

      const blob = jsonToXlsxBlob(allCurricula);
      const filename = `Curriculum_Template_24-25.xlsx`;
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/45 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl border border-gray-200 p-6 text-center">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Please wait while we process and save your XLSX file.
            </p>
          </div>
        </div>
      )}
